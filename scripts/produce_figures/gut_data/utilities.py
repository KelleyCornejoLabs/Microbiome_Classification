...
def plot_bars_oral(validation_60, validation_80, stratabionn_60_data, stratabionn_80_data, path=None):
...
def plot_bars_gut(validation_60, validation_80, stratabionn_60_data, stratabionn_80_data, entero_60_data, entero_80_data):
    fig, axes = plt.subplots(nrows=2, ncols=2, figsize=(10, 8))

    print(len(validation_60), len(stratabionn_60_data))
    print(len(validation_80), len(stratabionn_80_data))
    print(len(validation_60), len(entero_60_data))
    print(len(validation_80), len(entero_80_data))

    strata_60_accuracy = round(accuracy_test(validation_60, stratabionn_60_data) / 100, 3)
    strata_80_accuracy = round(accuracy_test(validation_80, stratabionn_80_data) / 100, 3)
    entero_60_accuracy = round(accuracy_test(validation_60, entero_60_data) / 100, 3)
    entero_80_accuracy = round(accuracy_test(validation_80, entero_80_data) / 100, 3)

    strata_60_f1 = round(f1_score(validation_60, stratabionn_60_data, average="weighted"), 3)
    strata_80_f1 = round(f1_score(validation_80, stratabionn_80_data, average="weighted"), 3)
    entero_60_f1 = round(f1_score(validation_60, entero_60_data, average="weighted"), 3)
    entero_80_f1 = round(f1_score(validation_80, entero_80_data, average="weighted"), 3)

    print(len(set(validation_60)), len(set(entero_60_data)), len(set(stratabionn_60_data)))
    strata_60_precision = round(precision_score(validation_60, stratabionn_60_data, average="weighted"), 3)
    strata_80_precision = round(precision_score(validation_80, stratabionn_80_data, average="weighted"), 3)
    entero_60_precision = round(precision_score(validation_60, entero_60_data, average="weighted"), 3)
    entero_80_precision = round(precision_score(validation_80, entero_80_data, average="weighted"), 3)

    strata_60_recall = round(recall_score(validation_60, stratabionn_60_data, average="weighted"), 3)
    strata_80_recall = round(recall_score(validation_80, stratabionn_80_data, average="weighted"), 3)
    entero_60_recall = round(recall_score(validation_60, entero_60_data, average="weighted"), 3)
    entero_80_recall = round(recall_score(validation_80, entero_80_data, average="weighted"), 3)
    ...
    # TODO: Should we use these?
    axes[0,0].set_ylim(entero_60_accuracy*0.9, 1)
    axes[0,1].set_ylim(entero_60_f1*0.9, 1)
    axes[1,0].set_ylim(entero_60_recall*0.9, 1)
    axes[1,1].set_ylim(entero_60_precision*0.9, 1)
    
    
    bars = [[None for _ in range(2)] for _ in range(2)]

    bars[0][0]=axes[0,0].bar([1,2,3,4], [strata_60_accuracy, strata_80_accuracy, 
                                entero_60_accuracy, entero_80_accuracy],
                                color=colors,
                                tick_label=["Stratabionn (60% training)", "Stratabionn (80% training)", 
                            "Enterotyper (60% training)", "Enterotyper (80% training)"])
    axes[0,0].set_xticklabels(["Stratabionn (60% training)", 
                                "Stratabionn (80% training)", "Enterotyper (60% training)", 
                                "Enterotyper (80% training)"], rotation=45, ha='right')

    bars[0][1]=axes[0,1].bar([1,2,3,4], [strata_60_f1, strata_80_f1, 
                            entero_60_f1, entero_80_f1],
                            color=colors,
                            tick_label=["Stratabionn (60% training)", "Stratabionn (80% training)", 
                            "Enterotyper (60% training)", "Enterotyper (80% training)"])
    axes[0,1].set_xticklabels(["Stratabionn (60% training)", 
                                "Stratabionn (80% training)", "Enterotyper (60% training)", 
                                "Enterotyper (80% training)"], rotation=45, ha='right')

    bars[1][0]=axes[1,0].bar([1,2,3,4], [strata_60_recall, strata_80_recall, 
                            entero_60_recall, entero_80_recall],
                            color=colors,
                            tick_label=["Stratabionn (60% training)", "Stratabionn (80% training)", 
                            "Enterotyper (60% training)", "Enterotyper (80% training)"])
    axes[1,0].set_xticklabels(["Stratabionn (60% training)", 
                                "Stratabionn (80% training)", "Enterotyper (60% training)", 
                                "Enterotyper (80% training)"], rotation=45, ha='right')

    bars[1][1]=axes[1,1].bar([1,2,3,4], [strata_60_precision, strata_80_precision, 
                            entero_60_precision, entero_80_precision],
                            color=colors,
                            data=[1,2,3,4,5,6],
                            tick_label=["Stratabionn (60% training)", "Stratabionn (80% training)", 
                            "Enterotyper (60% training)", "Enterotyper (80% training)"])
    axes[1,1].set_xticklabels(["Stratabionn (60% training)", 
                                "Stratabionn (80% training)", "Enterotyper (60% training)", 
                                "Enterotyper (80% training)"], rotation=45, ha='right')
    ...
    plt.tight_layout()
    # plt.title("Performance Metrics for all Methods")
    plt.savefig("fig_gut_bars.svg", format='svg')
    plt.savefig("fig_gut_bars.jpeg")
    plt.savefig("fig_gut_bars.pdf", format="pdf")
    plt.show()

def plot_confusion_oral(validation_80, stratabionn_80_data, path=None, dataset_name=None):
    fig, ax = plt.subplots(nrows=1, ncols=1, figsize=(5, 4))
    ...
    ax.set_title(f"{dataset_name} Dataset: 80% Training", loc='left')
...
def plot_confusion_gut(validation_80, stratabionn_80_data, entero_80_data):
    ...
    validation_80 = list(map(lbl_to_idx, validation_80))
    stratabionn_80_data = list(map(lbl_to_idx, stratabionn_80_data))
    entero_80_data = list(map(lbl_to_idx, entero_80_data))

    # strata, entero, strata - entero. All 80% 
    confmat_stratabionn = confusion_matrix(validation_80, stratabionn_80_data)
    confmat_entero = confusion_matrix(validation_80, entero_80_data)
    ...
    cms = [confmat_stratabionn, confmat_entero, confmat_stratabionn - confmat_entero]
    cmaps = ["Blues", "Blues", RedBlues]
    coords = [(0,0), (0,1), (0,2)]
    # tools = ["VALENCIA", "Stratabionn", "Random Forest", "Stratabionn - VALENCIA"]
    tools = ["A)", "B)", "C)"]
    ...
    for i in range(len(cms)):
        x, y = coords[i]
        disp = ConfusionMatrixDisplay(confusion_matrix=cms[i], display_labels=all_lbls)
        disp.plot(ax=axes[y], cmap=cmaps[i])
        axes[y].set_xticklabels(all_lbls, rotation=45)
        axes[y].set_title(tools[i], loc='left')
    
    plt.tight_layout()
    # plt.title("Confusion Matrices for all Methods")
    plt.savefig("fig_gut_confmat.svg", format='svg')
    plt.savefig("fig_gut_confmat.jpeg")
    plt.show()
...
def plot_hickey_valencia_comparison(france_data, hickey_data, common_cols, overlay=True, figure=4):
    ...
    # pacmap_data_df = pd.DataFrame()
    # pacmap_data_df["sampleID"] = list(france_data["sampleID"]) + list(hickey_data["sampleID"])
    # pacmap_data_df["study"] = ["ground truth"]*len(france_data)+["human microbiome compendium"]*len(hickey_data)
    # pacmap_data_df["X"] = X_transformed[:, 0]
    # pacmap_data_df["Y"] = X_transformed[:, 1]
    # pacmap_data_df["lbl"] = list(map(lambda x: universe[x], y_data))
    # pacmap_data_df = pd.concat([pacmap_data_df, X_data], axis=1)
    # pacmap_data_df.to_csv("PACMAP_DATA.csv", index=False)
    ...
    norm = mpl.colors.Normalize(vmin=0, vmax=len(unique_labels))
    def apply_col(x, opacity=0.05):
        label = universe[x]
    ...
    def only_sel(x, opacity=0.05, sel="ref"):
        label = universe[x]
        col = list(cmap(norm(x))) #unique_labels.index(label.split(" ")[0])
        if sel in universe[x]:
            col = (np.float64(col[0]),np.float64(col[1]),np.float64(col[2]), np.float64(opacity))
        else: col = (0,0,0,0)
        return col
    ...
    adjusted_cmap = ListedColormap([apply_col(i) for i in range(0, len(universe))])
    ref_only_cmap = ListedColormap([only_sel(i, sel="Bact_Phoc") for i in range(0, len(universe))])
    ...
    if overlay: axes[1].scatter(X_transformed[:, 0], X_transformed[:, 1], cmap=ref_only_cmap, marker="x", c=y_data, s=2.6)
    # plot_single_pacmap(X_data, y_data, universe, axes, divisions=[len(france_common_data), -1], marks=['+', 'x'])
...
def get_pacmap_csv(france_data, hickey_data, common_cols):
    ...
    embedding = pacmap.PaCMAP()
    X_transformed = embedding.fit_transform(all_data.drop(columns=["class","dataset","sampleID"]), init="pca")
...
def plot_gut_pacmaps(condordant_data, discondordant_data, figure=None):
    if figure == None:
        print("Please provide figure name")
        return

    fig, axes = plt.subplots(nrows=1, ncols=2, figsize=(12, 5))

    condordant_data_classification      = condordant_data["subCST"].astype("string")+" ref"
    discondordant_data_classification   = discondordant_data["subCST"].astype("string")
    
    condordant_data_no_lbl     = condordant_data.drop(columns=['subCST']) 
    discondordant_data_no_lbl  = discondordant_data.drop(columns=['subCST'])

    all_data = pd.concat([condordant_data_no_lbl, discondordant_data_no_lbl], axis=0)
    all_class = pd.concat([condordant_data_classification, discondordant_data_classification], axis=0)
    all_data["label"] = all_class
    all_data.to_csv("test.csv")
    ...
    X_data, y_data, universe = extract_numpy(all_data, "label", ["sampleID"])
    print(list(set(y_data)))
    ...
    pacmap_data_df = pd.DataFrame()
    pacmap_data_df["sampleID"] = all_data["sampleID"]
    pacmap_data_df["X"] = X_transformed[:, 0]
    pacmap_data_df["Y"] = X_transformed[:, 1]
    pacmap_data_df["lbl"] = y_data
    pacmap_data_df.to_csv("PACMAP_DATA.csv", index=False)
    ...
    sort_idxs = np.argsort(y_data)

    axes[0].set_title("A)", loc='left')
    axes[1].set_title("B)", loc='left')
    axes[0].scatter(X_transformed[sort_idxs, 0], X_transformed[sort_idxs, 1], cmap=opaque_cmap, marker="x", c=y_data[sort_idxs], s=2.6)
    axes[1].scatter(X_transformed[sort_idxs, 0], X_transformed[sort_idxs, 1], cmap=adjusted_cmap, marker="x", c=y_data[sort_idxs], s=2.6)
    # plot_single_pacmap(X_data, y_data, universe, axes, divisions=[len(france_common_data), -1], marks=['+', 'x'])
    ...
    plt.tight_layout()
    # plt.title("Confusion Matrices for all Methods")
    print(f"Saving to {figure}")
    plt.savefig(f"{figure}.svg", format='svg')
    plt.savefig(f"{figure}.jpeg")
    plt.show()
...
def entero_cmp_main(args):
    validation_60_path = args.validation_60
    validation_80_path = args.validation_80
    stratabionn_60_path = args.stratabionn_class_60
    stratabionn_80_path = args.stratabionn_class_80
    entero_60_path = args.entero_class_60
    entero_80_path = args.entero_class_80
    ...
    stratabionn_60_data = list(load_class(stratabionn_60_path)["subCST"])
    stratabionn_80_data = list(load_class(stratabionn_80_path)["subCST"])
    entero_60_data = list(load_class(entero_60_path, class_lbl='enterotype')["subCST"])
    entero_80_data = list(load_class(entero_80_path, class_lbl='enterotype')["subCST"])

    plot_bars_gut(validation_60, validation_80, stratabionn_60_data, stratabionn_80_data, entero_60_data, entero_80_data)

    plot_confusion_gut(validation_80, stratabionn_80_data, entero_80_data)
...
def fig_6_main(args):
    ...
    plot_bars_oral(validation_60_data, validation_80_data, stratabionn_60_data, stratabionn_80_data, path=path)

    plot_confusion_oral(validation_80_data, stratabionn_80_data, path=path, dataset_name=dataset_name)
...
def gut_classification_pacmap(args):
    condordant_class = args.condordant_class
    discondordant_class = args.discondordant_class

    condordant_data = pd.read_csv(condordant_class)
    discondordant_data = pd.read_csv(discondordant_class)

    plot_gut_pacmaps(condordant_data, discondordant_data, figure=args.fig)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--fig", help="name of figure", type=str, default=None)

    subparsers = parser.add_subparsers(dest='subcommand', help='Available subcommands')

    parser_fig_1_2 = subparsers.add_parser('fig_1_and_2', help='Figure 1 and 2 help')
    parser_fig_gut_cmp = subparsers.add_parser('fig_gut_cmp', help='Figure for gut enterotyper comparison help')
    ...
    parser_gut_pacmap = subparsers.add_parser('gut_pacmap', help='gut_pacmap help')
    ...
    fig_gut_cmp_group = parser_fig_gut_cmp.add_argument_group('Fig for gut (Enterotyper) comparison Options')
    fig_gut_cmp_group.add_argument("--entero-class-60", help="Classifications from VALENCIA", type=str)
    fig_gut_cmp_group.add_argument("--entero-class-80", help="Classifications from VALENCIA", type=str)
    fig_gut_cmp_group.add_argument("--stratabionn-class-60", help="Classifications from Stratabionn 60/20/20", type=str)
    fig_gut_cmp_group.add_argument("--stratabionn-class-80", help="Classifications from Stratabionn 80/10/10", type=str)
    fig_gut_cmp_group.add_argument("--validation-60", help="Classifications from 60% validaiton set", type=str)
    fig_gut_cmp_group.add_argument("--validation-80", help="Classifications from 80% validaiton set", type=str)
    ...
    # Argument group for gut pacmap
    gut_pacmap = parser_gut_pacmap.add_argument_group('gut_pacmap Options')
    gut_pacmap.add_argument("--condordant-class", help="Path to concordant data set", type=str)
    gut_pacmap.add_argument("--discondordant-class", help="Path to disconcordant data set", type=str)
    ...
    elif args.subcommand == 'gut_pacmap':
        gut_classification_pacmap(args)
    elif args.subcommand == 'fig_gut_cmp':
        entero_cmp_main(args)
    else:
        parser.print_help()
