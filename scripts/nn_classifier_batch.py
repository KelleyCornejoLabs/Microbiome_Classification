...
def train(classifier: nn.Sequential, X_train: torch.Tensor, y_train: torch.Tensor, 
    ...
    for epoch in range(max_epochs):
        ...
        for idx in range(0, len(X_train), batch_size):
            X = X_train[idx:idx+batch_size].to(device)
            y = y_train[idx:idx+batch_size].to(device)
    ...
    if train_model:
        ...
        if continue_train:
            ...
            train(classifier, X_train, y_train, X_test, y_test, lr, max_epochs, metrics_interval, thresh, 
                  ...
                  optim=optim, patience=args.patience, debug=debug, use_op=use_ordered_prevelance)
