...
# Load the data
def load_data_manghi(path: str) -> pd.DataFrame:
    # Load and transpose
    df = pd.read_csv(path)
    df = df.drop(columns=["HC_subCST"])
    return df
...
def print_elbow(data: np.array, out: str, n: int):
    inertias = []
    ...
    plt.savefig(out + "_elbow.jpeg")
    plt.savefig(out + "_elbow.svg", format='svg')
    plt.clf()

def print_silhouettes(data: np.array, out: str, n: int):
    scores = []
    ...
    plt.savefig(out + "_silhouette.jpeg")
    plt.savefig(out + "_silhouette.svg", format='svg')
    plt.clf()

# Plot silhouette graphs
# Reference: https://scikit-learn.org/stable/auto_examples/cluster/plot_kmeans_silhouette_analysis.html
def plot_silhouette(data: np.array, out: str, n: int):
    for n_clusters in range(2,n):
        ...
        plt.savefig(f"{out}_silhouette_{n_clusters}.jpeg")
        plt.savefig(f"{out}_silhouette_{n_clusters}.svg", format='svg')
        plt.clf()
...
if __name__ == "__main__":
    ...
    arguments.add_argument("-m", "--manghi", action=argparse.BooleanOptionalAction, help="Use manghi dataset", default=False)
    ...
    if args.graph:
        print_elbow(data, out, 10)
        plot_silhouette(data, out, 10)
        print_silhouettes(data, out, 10)
        exit(0)
