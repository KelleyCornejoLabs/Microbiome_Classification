...
def get_all_labels(path_train: str, path_test: str, debug: bool = False):
    ...
    try:
        ...
        print(f"ERR: Could not load input data from {path_train} or {path_test}")
        exit()
...
def load_file(path: str, expect_labeled: bool, drop: None|list[str] = [], 
    ...
    if labeled:
        ...
        # print("Lbls:", all_labels, "entries:", entries)
    ...
    normalized_data[normalized_data.isnull()] = 0#1.0e-5
...
def train(classifier: nn.Sequential, X_train: torch.Tensor, y_train: torch.Tensor, 
          ...
          debug: bool = False, patience: int = 50, write_file: bool = True, use_op = False, batch_size=32) -> float:
    ...
    if loss_type == "ce" and use_op:
        print("INFO: Using inverse prevelance weighting")
    ...
    if patience != 0:
        if batch_size > 0:
            patience = patience * (len(X_train) // batch_size)
        scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optim, factor=0.2, patience=patience)
    ...
    # Set model to train mode and move to device only once
    classifier.train()
    ...
    for epoch in range(max_epochs):
        ...
        # Training loss is average of individual losses
        ...
        if epoch % metrics_interval == 0:
            ...
            with torch.inference_mode():
                ...
                for idx, (X, y) in enumerate(test_dataloader):
                    ...
                    total_test_loss += test_loss.item()*batch_size # CHeck this
                    test_accuracy += accuracy_test(y_test, test_pred) * (len(X)/len(X_test))
    ...
    with torch.inference_mode():
        ...
        if mpl:
            prep = lambda x:list(map(lambda y:y.cpu(), x))
...
def load_model(path: str, keys: None|list[str] = None, return_features: bool = False, 
    ...
    # Load 'checkpoint'
    try:
        checkpoint = torch.load(path + "_nn.pt", weights_only=False)
    except:
...
def train_simpler_model(train_path: str, test_path: str, sorted_importances: dict[str, float], 
                        ...
                        models: int = 1, regex_remove:list[str] = [","], batch_size:int = 0) -> tuple[nn.Sequential, torch.Tensor, 
    ...
    for i in range(models):
        ...
        acc = train(Sclassifier, SX_train, Sy_train, SX_test, Sy_test, lr, max_epochs, metrics_interval, thresh, 
              ...
              Skeys, patience=patience, debug=debug, batch_size=batch_size)
...
if __name__ == "__main__":
    ...
    arguments.add_argument("-bs","--batch-size", type=int, help="Dropout layer parameter", default=0)
    ...
    use_ordered_prevelance = args.weight_inverse_proportional
    ...
    if train_model:
        ...
        # print(f"Ops: {ordered_prevelence}")
        # exit(0)
        ...
        if continue_train:
            ...
            train(classifier, X_train, y_train, X_test, y_test, lr, max_epochs, metrics_interval, thresh, 
                  ...
                  optim=optim, patience=args.patience, debug=debug, use_op=use_ordered_prevelance, batch_size=batch_size)
        ...
        if train_simple:
            ...
            Simple_classifier, SX_test, Sy_test, Sall_lbls = train_simpler_model(args.input_train, args.input_test, 
                                                                                 ...
                                                                                 norm=norm_fn, regex_remove=regex_remove, batch_size=batch_size)
